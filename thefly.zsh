...
{
    typeset fenvd="$1"
    printf %s ". $fenvd/.flyrc.d/thefly;_flogin_rc $fenvd >$fenvd/.zshrc;ZDOTDIR=$fenvd zsh -di "
}

function _fsource_plugins
{
    typeset p=${1:-*} plugin
    for plugin in $(ls $_fenvdir/.flyrc.d/${~p}/*.plugin.zsh 2>/dev/null);do
        . $plugin
    done
}
...
. ./.flyrc.d/thefly
_flogin_rc $destdir >$destdir/.zshrc
ZDOTDIR=$destdir exec zsh -di
EOF
}
