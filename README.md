...
![thefly_demo](https://github.com/user-attachments/assets/dea536d5-b256-4867-854f-d5d0fcd3e276)
...
need to test before adopting thefly ? Just try my own [flypack](https://github.com/joknarf/flypack) (temporary install in `/tmp/.fly.$UID`, xz needed):
```
...
for fish plugins, the sourced files are in `~/.fly.d/plugins/*/conf.d/*.fish`, (`thefly` will add `functions/completions` directories to fish if present). Need to execute `fly.fish source` after adding plugin to activate.
